...
export const goldentime = {
  eyebrow: 'A product of Arbiter · Cybersecurity',
  name: 'Goldentime',
  tagline: 'One product for the whole scam window, before the transfer and after.',
  lead: 'Goldentime runs on the phone and covers the two halves of a voice-phishing scam that are usually sold as separate products. While the call is live it reads the surfaces the platforms structurally cannot see \u2014 the messenger conversation and the bank transfer screen \u2014 and routes a warning to a trusted family member. If money goes anyway, the same app becomes the recovery copilot for the hour that decides whether it can be clawed back. Everything is analysed on the device, so no audio and no conversation leaves the phone.',
  problem: {
    label: 'The problem',
    body: 'The fastest-growing fraud of the decade is a phone call, and the people targeted most are elderly. The defences that exist watch the call and the text, but the scam is completed somewhere they cannot look: inside the messenger thread and on the banking app\u2019s transfer screen. And when it succeeds, the difference between recovering the money and losing it is measured in minutes \u2014 while the victim is panicked, and the family scrambling to help knows even less. Detection without recovery leaves the victim alone at the worst moment. Recovery without detection arrives after the loss. The window is one window; it was being covered by two half-products.',
  },
  how: {
    label: 'How it works',
    points: [
      { h: 'It reads the channels nobody else can', p: 'An on-device language model reads the messenger conversation and the pre-transfer bank screen, so it can flag a scam at the exact point of loss rather than from a blocklist that is always a step behind.' },
      { h: 'On-device, so nothing leaves the phone', p: 'The call and the chat are analysed locally. No audio and no conversation is sent to a server, which solves privacy and latency at the same time and lets the defence work where a network does not.' },
